...
## 2026-05-24 - [Security Hardening] Prevented Full Path Disclosure in Templates
**Vulnerability:** Several theme template files (e.g., 404.php, front-page.php, header.php, footer.php) could be accessed directly, potentially causing Full Path Disclosure (FPD) if PHP errors were displayed.
**Learning:** Classic WordPress themes don't have an inherent direct access protection or version hiding mechanism by default; it must be implemented manually in all entry point files, not just `functions.php`.
**Prevention:** Added `if ( ! defined( 'ABSPATH' ) ) { exit; }` to all entry point PHP files.
