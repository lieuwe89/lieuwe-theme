...
## 2026-06-16 - [Security Hardening] Prevented User Enumeration via oEmbed
**Vulnerability:** WordPress oEmbed responses include `author_name` and `author_url` fields by default. Unauthenticated users could query the oEmbed endpoint with a valid post URL and retrieve the author's username, which bypasses other user enumeration protections.
**Learning:** Preventing user enumeration in WordPress requires a multi-layered approach. Even when the REST API endpoints and author archives are disabled, core features like oEmbed can still leak sensitive user information.
**Prevention:** Hooked into the `oembed_response_data` filter to unset `author_name` and `author_url` from the oEmbed response data array before it is returned.
