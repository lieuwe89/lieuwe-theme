...
## 2026-05-20 - [Security Hardening] Prevented Version Leakage via Asset URLs and Added Permissions-Policy
**Vulnerability:** Even after disabling `wp_generator` meta tag and feed versioning, WordPress still exposed its version number as a `?ver=` query parameter on all native scripts and stylesheets loaded on the frontend, allowing attackers to fingerprint the WP version. Also, browser features were not restricted.
**Learning:** Hiding WordPress versioning requires a multi-faceted approach. Specifically stripping it from asset URLs requires hooking into `style_loader_src` and `script_loader_src`. Additionally, adding `Permissions-Policy` adds another layer of defense against compromised dependencies accessing device APIs.
**Prevention:** Wrote `lieuwe_remove_wp_version_strings()` function attached to asset loading filters, which uses `remove_query_arg('ver', $src)` if the version matches the core WP version. Also added a `Permissions-Policy` header to `lieuwe_add_security_headers()`.
