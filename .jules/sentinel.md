...
## 2026-06-25 - [Security Hardening] Prevented User Enumeration via author_name and Core Sitemaps
**Vulnerability:** Despite earlier protections blocking numeric author queries and REST API enumeration, user enumeration was still possible using `/?author_name=username` and via the WordPress core sitemaps feature (`/wp-sitemap-users-1.xml`), which automatically exposed usernames of authors with published content.
**Learning:** Security mechanisms in WordPress must defend against all methods of querying (such as specific query variables beyond just 'author') and built-in features that actively generate public endpoints, like WP 5.5+ XML Sitemaps.
**Prevention:** Always block the `author_name` URL parameter specifically alongside numeric ones. Actively filter `wp_sitemaps_add_provider` returning false for 'users' to disable user enumeration natively built into WP sitemaps.
