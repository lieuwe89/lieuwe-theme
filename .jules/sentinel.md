...
## 2026-08-10 - [Security Hardening] Prevented User Enumeration via oEmbed API
**Vulnerability:** Even when REST API and author query parameters are restricted, WordPress exposes user information (such as author name and URL) via the oEmbed API response. This allows attackers to enumerate valid usernames.
**Learning:** WordPress continually adds new features (like native oEmbed) that may inadvertently expose data previously secured through manual overrides. Security measures must account for these features that bypass older enumeration protections.
**Prevention:** Filter `oembed_response_data` to unset `author_name` and `author_url` from the returned data.
